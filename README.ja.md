...
- JavaScript のチェックは、ESLint 設定が検出された場合のみ実行されます。
- CSS、SCSS、Sass のチェックは、Stylelint 設定が検出された場合のみ実行されます。
- HTML のチェックは、htmlhint 設定が検出された場合のみ実行されます。
...
- JavaScript の保存時検証は、ESLint 設定が検出された場合に Prettier と ESLint を使用します。
- CSS、SCSS、Sass の保存時検証は、Stylelint 設定が検出された場合に Prettier と Stylelint を使用します。
- HTML の保存時検証は、htmlhint 設定が検出された場合に Prettier と htmlhint を使用します。
...
- `prepush/workspace` は、対応する設定が検出された場合に、ワークスペース内のファイルに対して ESLint、Stylelint、htmlhint も実行します。
...
| JavaScript ESLint | `eslint.config.js`、`eslint.config.mjs`、`eslint.config.cjs`、`eslint.config.ts`、`eslint.config.mts`、`eslint.config.cts`、`.eslintrc`、`.eslintrc.js`、`.eslintrc.cjs`、`.eslintrc.json`、`.eslintrc.yaml`、`.eslintrc.yml`、`.eslintrc.ts`、`.eslintrc.mts`、`.eslintrc.cts` のいずれか、または `eslintConfig` を含む `package.json` | これらの設定がない場合、JavaScript のチェックはスキップされます。 |
| CSS / SCSS / Sass Stylelint | `stylelint.config.js`、`stylelint.config.mjs`、`stylelint.config.cjs`、`stylelint.config.ts`、`stylelint.config.mts`、`stylelint.config.cts`、`.stylelintrc`、`.stylelintrc.js`、`.stylelintrc.cjs`、`.stylelintrc.json`、`.stylelintrc.yaml`、`.stylelintrc.yml`、`.stylelintrc.ts`、`.stylelintrc.mts`、`.stylelintrc.cts` のいずれか、または `stylelint` を含む `package.json` | これらの設定がない場合、CSS 系のチェックはスキップされます。 |
| HTML htmlhint | `.htmlhintrc`、`.htmlhintrc.js`、`.htmlhintrc.cjs`、`.htmlhintrc.json`、`.htmlhintrc.yaml`、`.htmlhintrc.yml` のいずれか、または `htmlhint` を含む `package.json` | これらの設定がない場合、HTML のチェックはスキップされます。 |
