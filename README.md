...
- When `mamori-inspector.saveCheckSkipFormatters` is set to `true`, all formatters (Spotless, Prettier, ESLint auto-fix) are skipped during automatic save checks. Static analysis checkers still run and diagnostics are still reported. The default is `false`. This setting is useful when an AI coding tool is editing files, because save-time formatting can cause the AI to encounter mismatched file content on subsequent edits.
- The commands `Mamori Inspector: Enable Save Formatters` and `Mamori Inspector: Disable Save Formatters` toggle `mamori-inspector.saveCheckSkipFormatters` per workspace folder without opening the settings UI.
