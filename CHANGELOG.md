...
### Fixed
- 保存時チェック実行後、対象ファイル以外のファイルの Diagnostics が消える問題を修正しました。
- フォーマッター変更後の再チェックでステータスバー通知が再表示されない問題を修正しました。
- stale な pre-push 結果を検出した際、手動チェックの Diagnostics を誤って消去していた問題を修正しました。
- `Set Up Managed Tools` 実行時に stdio ストリームが生成されない子プロセスで処理が完了しない問題を修正しました。
- 拡張ホスト起動直後に runtime 同期と既存結果処理を同期実行してホストをブロックする問題を修正しました。
- Diagnostics のメッセージにコロンが含まれる場合に重複判定が誤る問題を修正しました。
...
### Fixed
- Windows CI で `eslint.config.mjs` を使う TypeScript ファイルの保存時チェックテストがモカのデフォルトタイムアウト 2000ms を超えて失敗する問題を修正しました。
...
### Fixed
- 保存時や手動実行で fresh な Checkstyle / PMD 出力が得られなかった場合に、古い既存 report を再利用して stale な診断を Problems へ残してしまう問題を修正しました。
- stale report を再利用しないことを確認する CLI 回帰テストを追加しました。
...
- `setup` 実行中に CLI が `mamori: setup installing=<tool>` を逐次出力し、拡張側が現在導入中のツール名を進捗通知へ反映できるようにしました。

### Fixed
- `Setup Managed Tools` が短時間で完了する環境でも、進捗通知が即座に閉じて見えなくなりにくいよう最低表示時間を設けました。

## [0.2.45] - 2026-04-10

### Fixed
- 保存時に `eslint --fix` などの formatter がファイル内容を書き換えた場合でも、Mamori Inspector が最新内容で保存時チェックを再実行し、古い診断を Problems へ残さないようにしました。
- ドキュメント編集後に、同一ファイルへ残っていた stale な save / manual / pre-push Diagnostics を消去するようにし、現在内容と一致しない古い指摘がエディタや Problems に残る問題を修正しました。
- stale な保存時診断と pre-push 診断が編集で消えることを確認する拡張回帰テストを追加しました。
